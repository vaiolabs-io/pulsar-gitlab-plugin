...
class GitLabClient {
   ...
   */
  async getJobLog (projectId, jobId, { offset = 0, etag = null, signal = null, decoder = null } = {}) {
    ...
    // A byte range starts and ends wherever the byte count landed, which may
    // be halfway through a multi-byte character. Slicing the Buffer rather
    // than the string is not enough on its own: the split shows up at the
    // seam between one poll and the next. A StringDecoder held by the caller
    // holds those few bytes back and emits the character once the rest of it
    // arrives. Without one, every such seam becomes a replacement character.
    const slice = useOffset ? buffer : buffer.slice(Math.min(offset, buffer.length));
    const text = decoder ? decoder.write(slice) : slice.toString('utf8');
    const totalBytes = useOffset ? offset + buffer.length : buffer.length;

    // Judge "nothing new" on bytes, not on text: a chunk can be a single
    // incomplete character, which decodes to nothing but is still progress.
    return { text, totalBytes, etag: nextEtag, unchanged: slice.length === 0 };
  }
