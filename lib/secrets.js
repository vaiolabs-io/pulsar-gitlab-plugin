 ...
 *
 * safeStorage is a main-process API. @electron/remote proxies it, and Pulsar
 * 1.132 enables remote for every window with no module filter. Third-party
 * packages cannot resolve @electron/remote out of Pulsar's own node_modules
 * (pulsar-edit/pulsar#1372), so we try the older require('electron').remote
 * shim first - that one does resolve, at the cost of a deprecation warning.
 */
