...
let cachedAvailable;
let cachedBackend;
let lastSafeStorageError = null;

/**
 * Get at Electron's safeStorage from a renderer.
 *
 * safeStorage is a main-process API; @electron/remote proxies it over IPC.
 * Pulsar 1.132 enables remote for every window with no module filter, and
 * safeStorage is in the proxied module list.
 *
 * We use our own pinned copy of @electron/remote, and only that. A third-party
 * package cannot resolve Pulsar's bundled copy (pulsar-edit/pulsar#1372), which
 * is why it is in our dependencies. It is pure JS, so an Electron bump does not
 * mean a native rebuild.
 *
 * We deliberately do NOT touch require('electron').remote. It is gone in a real
 * Pulsar window - accessing it throws "electron.remote is deprecated and
 * removed!" - even though the spec environment still answers for it. Reaching
 * for it as a fallback bought nothing and put a scary message in the console.
 */
function safeStorage () {
  ...
  try {
    const candidate = require('@electron/remote').safeStorage;
    if (candidate && typeof candidate.isEncryptionAvailable === 'function') {
      cachedSafeStorage = candidate;
    } else {
      lastSafeStorageError = 'This Pulsar build does not expose safeStorage.';
    }
  } catch (err) {
    // No keyring route. Not fatal: the connect dialog offers an environment
    // variable or the GitLab CLI, both of which store nothing at all.
    lastSafeStorageError = err.message;
  }
  return cachedSafeStorage;
}

/** Why encrypted storage is unavailable, for the connect dialog to show. */
function keyringUnavailableReason () {
  keyringAvailable();
  if (lastSafeStorageError) return lastSafeStorageError;
  const backend = keyringBackendName();
  if (backend === 'basic_text') {
    return 'Your desktop has no keyring running, so Electron would "encrypt" with a key that is public knowledge.';
  }
  return null;
}
