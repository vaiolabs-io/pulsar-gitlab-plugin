...
const { JobsTile } = require('./views/jobs-tile');
const { StagesTile } = require('./views/stages-tile');
...
// How long to wait for Pulsar's "initial packages are up" signal before
// starting the status light anyway. Long enough to be clear of the activation
// path, which is what crashed the renderer when this work was done inline.
const STATUS_START_FALLBACK_MS = 3000;
...
module.exports = {
  activate (state) {
    ...
    this.jobsTile = null;
    this.jobsBarTile = null;
    this.stagesTile = null;
    this.stagesBarTile = null;
    this.poller = null;
    this.connections = null;
    this.context = null;
    this.lastNotifiedStatus = new Map();

    // A pipeline the user picked out of the Recent list. While this is set the
    // panel shows that pipeline instead of the branch's latest, so its job
    // logs are one click away. The status bar keeps showing the branch, which
    // is what the user actually wants watched.
    this.selectedPipeline = null;
    this.serializedViewState = (state && state.view) || null;
    this.statusStartTimer = null;
    this.statusFallbackTimer = null;
    this.statusStartPending = false;
    ...
    this.subscriptions.add(new Disposable(() => {
      ...
      this.warmUpTimer = null;
      if (this.statusStartTimer) clearTimeout(this.statusStartTimer);
      this.statusStartTimer = null;
      if (this.statusFallbackTimer) clearTimeout(this.statusFallbackTimer);
      this.statusFallbackTimer = null;
      this.statusStartPending = false;
    }));
  },
  ...
  deactivate () {
    ...
    if (this.jobsBarTile) {
      this.jobsBarTile.destroy();
      this.jobsBarTile = null;
    }
    if (this.jobsTile) {
      this.jobsTile.destroy();
      this.jobsTile = null;
    }
    if (this.stagesBarTile) {
      this.stagesBarTile.destroy();
      this.stagesBarTile = null;
    }
    if (this.stagesTile) {
      this.stagesTile.destroy();
      this.stagesTile = null;
    }
    if (this.context) {
  ...
  /**
   * Bring the stores up for the status light, without doing it on the
   * activation path.
   *
   * consumeStatusBar runs while the window is still activating packages, and
   * ensureStarted() reads the connection file and asks the OS keyring to
   * decrypt a token. Calling it straight from there took the whole renderer
   * down with a native crash - which is the same hazard warmUpTimer exists to
   * dodge, one screen up. Specs cannot catch it: under `pulsar --test` there
   * are no connections, so nothing ever reaches the keyring.
   *
   * So: wait for Pulsar to say the initial packages are done, then start on
   * the next tick after that. If the package was activated later by a command,
   * that event has already been and gone, and the timer alone is enough.
   */
  startForStatusBar () {
    if (this.connections || this.statusStartPending) return;
    this.statusStartPending = true;
    ...
    const begin = () => {
      ...
      this.statusStartTimer = setTimeout(() => {
        this.statusStartTimer = null;
        this.statusStartPending = false;
        // The light can have been switched off while we waited.
        if (!this.statusTile && !this.jobsTile && !this.stagesTile) return;
        if (!this.wantsStatusBar()) return;
        try {
          this.ensureStarted();
          this.refresh({ force: true });
        } catch (err) {
    ...
    const canWait = packages &&
      typeof packages.onDidActivateInitialPackages === 'function' &&
      typeof packages.hasActivatedInitialPackages === 'function';

    if (canWait && !packages.hasActivatedInitialPackages()) {
      this.subscriptions.add(packages.onDidActivateInitialPackages(() => begin()));
      // Belt and braces. If that signal never arrives - it does not fire at
      // all under the spec harness - starting late is fine, but never
      // starting leaves a tile that can only ever say "GitLab" and nothing
      // else, which is the bug this whole method exists to fix.
      this.statusFallbackTimer = setTimeout(() => begin(), STATUS_START_FALLBACK_MS);
    } else {
      begin();
    }
  },
  ...
  consumeStatusBar (statusBar) {
    ...
    this.jobsTile = new JobsTile({ onClick: () => this.togglePanel() });
    this.jobsBarTile = statusBar.addRightTile({ item: this.jobsTile.element, priority: 151 });

    this.stagesTile = new StagesTile({ onSelect: (name) => this.revealStage(name) });
    this.stagesBarTile = statusBar.addRightTile({ item: this.stagesTile.element, priority: 152 });

    // Honour the settings. showStatusBar shipped in 0.1.0 declared in
    // configSchema and asserted in a spec, but nothing read it - so toggling
    // it did nothing.
    //
    // A light nothing feeds is a decoration: without the start below, a tile
    // sat on its constructor's idle state until the panel was opened once.
    // Deferred, because this runs inside activation - see startForStatusBar.
    this.subscriptions.add(atom.config.observe('gitlab-pipelines.showStatusBar', (show) => {
      ...
      if (show) this.startForStatusBar();
    }));

    this.subscriptions.add(atom.config.observe('gitlab-pipelines.showJobProgress', (show) => {
      if (!this.jobsTile) return;
      this.jobsTile.setVisible(show);
      if (show) this.startForStatusBar();
    }));

    this.subscriptions.add(atom.config.observe('gitlab-pipelines.showStagesInStatusBar', (show) => {
      if (!this.stagesTile) return;
      this.stagesTile.setVisible(show);
      if (show) this.startForStatusBar();
    }));
    ...
    return new Disposable(() => {
      ...
      this.statusBarTile = null;
      if (this.jobsBarTile) this.jobsBarTile.destroy();
      this.jobsBarTile = null;
      if (this.stagesBarTile) this.stagesBarTile.destroy();
      this.stagesBarTile = null;
    });
  },

  /** Is either status bar tile switched on? Nothing starts for the bar if not. */
  wantsStatusBar () {
    return atom.config.get('gitlab-pipelines.showStatusBar') === true ||
      atom.config.get('gitlab-pipelines.showJobProgress') === true ||
      atom.config.get('gitlab-pipelines.showStagesInStatusBar') === true;
  },

  /** Open the panel and put the named stage in view. */
  async revealStage (name) {
    this.ensureStarted();
    await atom.workspace.open(PIPELINES_URI);
    if (this.view && name) this.view.scrollToStage(name);
  },
  ...
  startPolling () {
    ...
    this.poller = new Poller({
      ...
      onStateChange: (state) => {
        ...
        if (state === 'offline') {
          if (this.statusTile) this.statusTile.setOffline();
          if (this.jobsTile) this.jobsTile.setOffline();
          if (this.stagesTile) this.stagesTile.setOffline();
        }
      },
  ...
  handlePollError (err, failures) {
    ...
    if (this.jobsTile) this.jobsTile.setOffline(err.message);
    if (this.stagesTile) this.stagesTile.setOffline(err.message);
  ...
  async refresh ({ force = false } = {}) {
    ...
    if (!context || !context.connectionId) {
      ...
      const reason = (this.connections ? this.connections.all().length : 0) === 0
        ? 'No GitLab connection yet - click to add one.'
        : 'No GitLab remote in this project.';
      if (this.statusTile) this.statusTile.setInactive(reason);
      if (this.jobsTile) this.jobsTile.setInactive(reason);
      if (this.stagesTile) this.stagesTile.setInactive(reason);
      return;
    }
    ...
    const [detail, list, schedules, pinned] = await Promise.all([
      this.loadDetail(client, context, project),
      client.listPipelines(project.id, { perPage: 10 }),
      this.loadSchedules(client, project),
      this.loadPinnedDetail(client, project)
    ]);

    // The pipeline the user picked wins in the panel; the tiles and the finish
    // notification stay on the branch, because that is what they are for.
    if (pinned === null && this.selectedPipeline) this.selectedPipeline = null;
    const shown = pinned || detail;

    if (this.view) {
      this.view.update(Object.assign(baseModel, {
        project,
        detail: shown,
        pinnedPipelineIid: pinned ? pinned.iid : null,
    ...
    if (this.jobsTile) {
      if (detail) this.jobsTile.setPipeline(detail);
      else this.jobsTile.setIdle();
    }
    if (this.stagesTile) {
      if (detail) this.stagesTile.setPipeline(detail);
      else this.stagesTile.setIdle();
    }
  ...
  /**
   * The pipeline the user picked out of the Recent list, if there is one.
   *
   * Three different returns, and the caller relies on the difference:
   *   undefined - nothing is pinned, carry on with the branch.
   *   null      - something is pinned but GitLab will not give it to us any
   *               more, so the caller unpins it rather than showing a stale
   *               pipeline for ever.
   *   an object - the detail to display.
   */
  async loadPinnedDetail (client, project) {
    const pinned = this.selectedPipeline;
    if (!pinned) return undefined;

    try {
      const detail = await client.pipelineDetail(project.path_with_namespace, pinned.iid);
      if (detail) return detail;

      // Same GraphQL-then-REST fallback as the branch pipeline: an older
      // self-hosted instance missing one field fails the whole query.
      if (pinned.id === undefined || pinned.id === null) return null;
      const [summary, jobs] = await Promise.all([
        client.getPipeline(project.id, pinned.id),
        client.listJobs(project.id, pinned.id)
      ]);
      if (!summary) return null;
      return restDetailShape(summary, jobs);
    } catch (err) {
      // A deleted pipeline, or one in a project we lost access to. Unpin it
  ...
  async handleViewRequest (event) {
    try {
      switch (event.action) {
        ...
        case 'select-pipeline': return await this.selectPipeline(event.pipeline);
        case 'clear-selection': return await this.clearSelection();
  ...
  /**
   * Clicking a pipeline in the Recent list. The point of this package is to
   * keep you in the editor, so the default is to show that pipeline's stages
   * and jobs in the panel - from there one click on a job opens its runner
   * log. The setting flips it back to opening GitLab in a browser, and the
   * per-row link button does that on demand either way.
   */
  async selectPipeline (pipeline) {
    if (!pipeline) return;
    if (atom.config.get('gitlab-pipelines.recentPipelineClick') === 'browser') {
      return this.openInBrowser(pipeline);
    }
    const iid = pipeline.iid || pipeline.id;
    if (iid === undefined || iid === null) return this.openInBrowser(pipeline);

    // Keep the numeric id too: the REST fallback in loadPinnedDetail needs it,
    // and the Recent list comes from REST so it always carries both.
    this.selectedPipeline = { iid, id: pipeline.id, ref: pipeline.ref || null };
    await this.refresh({ force: true });
  },

  /** Back to whatever is newest on the current branch. */
