...
module.exports = {
  ...
  startForStatusBar () {
    ...
    const begin = () => {
      ...
      this.statusStartTimer = setTimeout(() => {
        ...
        if (!atom.config.get('gitlab-pipelines.showStatusBar')) return;
        try {
          this.ensureStarted();
          this.refresh({ force: true });
        } catch (err) {
  ...
  consumeStatusBar (statusBar) {
    ...
    this.subscriptions.add(atom.config.observe('gitlab-pipelines.showStatusBar', (show) => {
      ...
      // A status light nothing feeds is a decoration: without this the tile
    ...
    if (!context || !context.connectionId) {
      ...
      if (this.statusTile) {
        this.statusTile.setInactive(
          (this.connections ? this.connections.all().length : 0) === 0
