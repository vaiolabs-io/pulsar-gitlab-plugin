...
/**
 * Pull remote names out of a git config file.
 *
 * Only the names: the URLs are read back through getConfigValue, so git's own
 * resolution applies rather than a second-guess of its file format here.
 *
 * @param {string} text - the contents of a .git/config
 * @returns {string[]} names in the order they appear
 */
function remoteNamesIn (text) {
  const names = [];
  const section = /^[^\S\n]*\[[^\S\n]*remote[^\S\n]+"([^"]+)"[^\S\n]*\]/gm;
  let match;
...
class ProjectContext {
  ...
  remoteNamesOf (repository) {
    ...
    try {
      return remoteNamesIn(fs.readFileSync(configPath, 'utf8'));
    } catch (err) {
...
module.exports = { ProjectContext, remoteNamesIn, gitConfigPathFor };
