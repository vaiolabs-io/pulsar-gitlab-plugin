...
// Stages that have not produced a result yet. Drawn as an outline rather than a
// filled circle, so "still to come" cannot be mistaken for "done".
const NOT_RUN_YET = new Set([
  'created', 'pending', 'manual', 'scheduled', 'preparing',
  'waiting_for_resource', 'waiting_for_callback', 'skipped'
]);

function notRunYet (status) {
  return NOT_RUN_YET.has(typeof status === 'string' ? status.toLowerCase() : '');
}
...
class StagesTile {
  ...
  render (specs) {
    ...
    this.dots = specs.map((spec) => {
      ...
      dot.href = '#';
      // A circle drawn in CSS rather than an octicon glyph: at this size a
      // dot reads as a stage marker, where a tick or a cross reads as the
      // whole pipeline's verdict. Colour still comes from the status class.
      dot.className = `gl-sb-stage ${look.klass}` +
        (notRunYet(spec.status) ? ' gl-sb-stage-open' : '') +
...
module.exports = { StagesTile, dotsFor, worstStatus, notRunYet, MAX_DOTS };
