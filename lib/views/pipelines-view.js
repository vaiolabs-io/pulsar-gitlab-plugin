...
class PipelinesView {
  ...
  renderStageStrip (detail) {
    ...
    stages.forEach((stage, index) => {
      ...
      if (!cell) {
        cell = this.buildStripCell(stage);
  ...
  buildStripCell (stage) {
    ...
    return { root, dot, name, tooltip: null, tooltipText: null };
  }
  ...
  patchStripCell (cell, stage) {
    ...
    // Only rebuild the tooltip when its text actually changed. Re-adding one
    // every poll would stack disposables on the same element.
    if (cell.tooltipText !== summary) {
      if (cell.tooltip) cell.tooltip.dispose();
      cell.tooltip = atom.tooltips.add(cell.root, { title: summary });
      cell.tooltipText = summary;
    }
  }
