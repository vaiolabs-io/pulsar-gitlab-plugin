...
class PipelinesView {
  constructor (state = {}) {
    ...
    this.model = {
      ...
      project: null,
      detail: null,
      // Set when the user picked a pipeline out of the Recent list, so the
      // "Current branch" section knows it is showing something else.
      pinnedPipelineIid: null,
      pipelines: [],
      schedules: [],
      canWrite: false,
      error: null,
  ...
  renderCurrent () {
    ...
    this.renderPinnedHeader(section);
  ...
  /**
   * When the user has picked a pipeline out of the Recent list, this section
   * is no longer showing the current branch, and saying so is the difference
   * between a feature and a confusing bug. The way back is a button in the
   * section header, which stays reachable even when the section is collapsed.
   */
  renderPinnedHeader (section) {
    const iid = this.model.pinnedPipelineIid;
    const signature = iid === null || iid === undefined ? '' : String(iid);
    if (section.actions.dataset.pinned === signature) return;
    section.actions.dataset.pinned = signature;
    section.actions.textContent = '';

    if (!signature) {
      setText(section.label, 'Current branch');
      return;
    }
    setText(section.label, `Pipeline #${signature}`);
    section.actions.appendChild(button('', {
      title: 'Back to the current branch',
      icon: 'arrow-left',
      className: 'btn btn-sm',
      onClick: () => this.request('clear-selection')
    }));
  }
  ...
  buildPipelineRow () {
    ...
    const actions = el('span', 'gl-pipeline-row-actions btn-group btn-group-sm');
    root.appendChild(icon);
    ...
    // Whatever the row click is set to do, GitLab itself stays one click away.
    // button() stops the event, so this never also triggers the row.
    actions.appendChild(button('', {
      title: 'Open this pipeline on GitLab',
      icon: 'link-external',
      className: 'btn btn-sm',
      onClick: () => this.request('pipeline-open', { pipeline: this.pipelineData.get(root) })
    }));
    return { root, icon, title, ref, when, actions };
  }

  patchPipelineRow (row, pipeline) {
    // Read through the WeakMap in the handlers rather than closing over this
    // call's `pipeline`, which the next poll replaces.
    this.pipelineData.set(row.root, pipeline);

    const look = lookOf(pipeline.status);
    const iid = pipeline.iid || pipeline.id;
    const selected = this.model.pinnedPipelineIid !== null &&
      this.model.pinnedPipelineIid !== undefined &&
      String(this.model.pinnedPipelineIid) === String(iid);

    setClass(row.icon, `icon icon-${look.icon} ${look.klass}`);
    setText(row.title, `#${iid}`);
    setText(row.ref, pipeline.ref || '');
    setText(row.when, humanAgo(pipeline.updated_at || pipeline.created_at));
    setClass(row.root, `gl-pipeline-row${selected ? ' gl-pipeline-row-selected' : ''}`);
    ...
    if (row.root.dataset.pipelineId !== String(pipeline.id)) {
      ...
      row.root.onclick = () => this.request('select-pipeline', { pipeline: this.pipelineData.get(row.root) });
    }
  }
